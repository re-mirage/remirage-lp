...
export function ProjectsList({ projects, team }: ProjectsListProps) {
  const [isPending, startTransition] = useTransition()
  const [search, setSearch] = useQueryState('search', { startTransition })
  const [techFilter, setTechFilter] = useQueryState('tech', { startTransition })
  const [teamFilter, setTeamFilter] = useQueryState('team', { startTransition })
  const [categoryFilter, setCategoryFilter] = useQueryState('category', { startTransition })

  const updateSearch = (value: string) => {
    startTransition(() => {
      setSearch(value, { scroll: false, shallow: true })
    })
  }

  const updateTechFilter = (value: string) => {
    startTransition(() => {
      setTechFilter(value === 'all' ? null : value, { scroll: false, shallow: true })
    })
  }

  const updateTeamFilter = (value: string) => {
    startTransition(() => {
